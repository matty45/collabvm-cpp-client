...
namespace cvm
{
	...
	void server::add_user(const QString& username, user::rank rank)
	{
		...
		if (m_user_list_model->find_user_by_username(username) != nullptr) {
		...
		m_user_list_model->append(new_user);
		m_user_count++;
	...
	{
		user* user = m_user_list_model->find_user_by_username(username);
		if (user)
		{
			qDebug() << "WS: User" << username << "left server" << m_name;
			m_user_list_model->remove(user);
			m_user_count--;
		}
	}
	...
	{
		user* u = m_user_list_model->find_user_by_username(old_username);
		if (u) {
	...
	void server::connect_to_server(bool skip_list_request)
	{

		m_skip_request_list = skip_list_request;
	...
	void server::disconnect_from_server()
	{
		...
		m_is_connected = false;
		qDebug() << "WS: Disconnecting from server:" << m_name;
	}
